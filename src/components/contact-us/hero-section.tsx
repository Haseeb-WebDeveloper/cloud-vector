...
export default function ContactHeroSection({
  ...
  emailLink,
  emailAddress,
  heroImage = "/hero-images/Contact%20us.png",
}: ContactHeroSectionProps) {
  const gradientButtonClass =
    "bg-gradient-to-r from-[#FF9900]/90 to-[#E85409]/90 hover:from-[#FF9900] hover:to-[#E85409] text-white border-0 shadow-none";

  return (
    <div className="relative min-h-[90vh] flex items-center justify-center overflow-visible pt-20 pb-20">
      <Image
        src={heroImage}
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 py-16 relative z-10">
        <div className="flex flex-col items-center text-center gap-8 max-w-4xl mx-auto">
          <h1
            ...
            style={{
              backgroundImage:
                "linear-gradient(90deg, var(--primary) 70%, #fff 100%)",
            }}
            dangerouslySetInnerHTML={{ __html: mainHeading }}
          />

          <div className="space-y-4 text-foreground/90">
            <p className="text-lg lg:text-xl leading-relaxed">{subheading}</p>
            <p className="text-base lg:text-lg leading-relaxed">
              {description}
            </p>
          </div>

          <ul className="flex flex-row flex-wrap justify-center gap-4 w-full text-center mt-4">
            {bullets.map((item: string) => (
              <li
                key={item}
                className="flex items-center gap-2 border border-white rounded-full px-2 py-2 transition-all duration-300 hover:shadow-[0_0_20px_rgba(255,153,0,0.6),0_0_40px_rgba(255,153,0,0.4)] hover:scale-105 cursor-default bg-black/20 backdrop-blur-sm"
              >
          ...
          <div className="flex flex-row flex-wrap justify-center items-center gap-4 pt-6 w-full">
            {ctaButtons.map((button: any, index: number) => {
              ...
              if (
                ...
                button.url?.includes("meeting")
              ) {
                IconComponent = <VideoIcon />;
              } else if (
                button.label?.toLowerCase().includes("whatsapp") ||
                button.url?.includes("whatsapp")
              ) {
                IconComponent = <WhatsAppIcon />;
              } else if (
                button.label?.toLowerCase().includes("signup") ||
                button.url?.includes("app.cloudvictor")
              ) {
                IconComponent = (
                  <svg
                    width="20"
            ...
            >
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center border-none shadow-none ${gradientButtonClass} transition-all duration-300 hover:shadow-[0_0_20px_rgba(255,153,0,0.6),0_0_40px_rgba(255,153,0,0.4)]`}
              >
