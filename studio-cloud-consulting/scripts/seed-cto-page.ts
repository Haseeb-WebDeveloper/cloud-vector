...
async function seedCTOPage() {
  ...
  const heroImagePath = '/hero-images/CTO.png'
  ...
  const heroSection = {
    ...
    subheading:
      'Savings that Sustain. Security that Proves. Speed & Reliability that boost revenue.',
  ...
  const howWeSolveSection = {
    ...
    subtitle:
      'Watch how we transform your pain points into powerful solutions through our innovative approach',
    painPoints: [
      {
  ...
  console.log(
    `  HowWeSolve: ${howWeSolveSection.painPoints.length} pain points, ${howWeSolveSection.solutions.length} solutions`,
  )
  ...
  const ctoPageDoc: any = {
    ...
    metaDescription:
      'Transform your AWS infrastructure with Cloud Victor. Optimize costs, enhance security, automate releases, and improve performance.',
  }

  // Debug: Log a sample of the structure
  console.log('\n🔍 Sample data structure:')
  console.log(
    'Hero section mainHeading:',
    ctoPageDoc.heroSection.mainHeading?.substring(0, 50) + '...',
  )
  ...
  try {
    ...
    console.log(`  - Client section: ✅ (${clientSection.stats.length} stats)`)
    console.log(
      `  - HowWeSolve section: ✅ (${howWeSolveSection.painPoints.length} pain points, ${howWeSolveSection.solutions.length} solutions)`,
    )
    console.log(`  - HowItWorks section: ✅ (${tabsWithImages.length} tabs)`)
    console.log(`  - Stats section: ✅ (${statsSection.stats.length} stats)`)
    console.log(
      `  - GetStarted section: ✅ (${getStartedSection.chips.length} chips, ${getStartedSection.ctaButtons.length} CTA buttons)`,
    )
